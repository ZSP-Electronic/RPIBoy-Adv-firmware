...
class CommandInterface:
    ...
    def readMemory(self, addr, lng):
        data = []
        if usepbar:
            widgets = ['Reading: ', Percentage(), ' ', Bar(), ' ', FileTransferSpeed()]
    ...
    def writeMemory(self, addr, data):
        ...
        lng = len(data)
        if usepbar:
            widgets = ['Writing: ', Percentage(), ' ', Bar(), ' ', FileTransferSpeed()]
            pbar = ProgressBar(widgets=widgets, maxval=lng, term_width=79).start()
...
class CommandProgrammer(CommandInterface):

    def __init__(self, reset_pin, boot_pin):
        ...
        GPIO.setup(self.boot_pin, GPIO.OUT, initial = 0)

        self.toggle = 1

    def reset(self):
        if self.toggle == 1:
            GPIO.output(self.boot_pin, 1)
            self.toggle = 0
        else:
            GPIO.output(self.boot_pin, 0)
            self.toggle = 1
...
def usage():
    print """Usage: %s [-hqVewvr] [-l length] [-p port] [-b baud] [-a addr] [-g addr] [-P RSTgpio,BOOTgpio] [file.bin]
        -h          This help
        -q          Quiet
        -V          Verbose
        -e          Erase
        ...
        -P          Program Pins (Reset GPIO,Boot GPIO)
...
if __name__ == "__main__":
    ...
    try:
        opts, args = getopt.getopt(sys.argv[1:], "hqVewvrp:b:a:l:g:P:")
    ...
    for o, a in opts:
        if o == '-V':
            ...
            rpi_program = True
            gpio = a.split(',')
            conf['resetIO'] = eval(gpio[0])
            conf['bootIO'] = eval(gpio[1])
        else:
            assert False, "unhandled option"

    if rpi_program is True:
        cmd = CommandProgrammer(conf['resetIO'], conf['bootIO'])
    else:
